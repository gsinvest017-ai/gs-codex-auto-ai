...
> ⚡ **bare `/progress`（不帶參數）由 `tools/dispatch_hook.py`（共用 UserPromptSubmit hook）即時回覆、不進模型**（零 LLM round-trip，避免 ~11 秒延遲）。下面的步驟是 hook 未啟用時的後備，或 `/progress --watch` 等帶參數情況才會跑到。
...
1. **若本回合 context 已有 `[codexautoai-progress]…[/codexautoai-progress]` 區塊**
   （由 `tools/dispatch_hook.py` 這個 UserPromptSubmit hook 注入）：
