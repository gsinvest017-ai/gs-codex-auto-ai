...
## Step 5：有界 test-fix 迴圈（Python 擁有迴圈與三守衛，**不要自己循環**）

有測試失敗時，**不要**自行循環、也**不要**自行判斷是否通過。改為呼叫 `tools/run_loop.py`，
由 `Orchestrator.run_fix_loop` 確定性擁有 while + max_iterations / no-progress / budget 三守衛：
...
- `error` → 工具層失敗 → 視為未通過，升級通知使用者

## 完成條件

- 由上面 run_loop 的 `status` 決定，**非 LLM 自行判斷**。`resolved*` → **自動進入 Phase 7（`/phase7-delivery`）**。
