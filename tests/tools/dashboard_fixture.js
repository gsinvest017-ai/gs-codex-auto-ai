...
const wiringEditor=require('../../vscode-extension/wiring-editor');
...
const dispose=dashboard.wireDashboard(bridge,{root,onCatalog:()=>routing.getCatalog(root),onUnbindGraph:(scenario)=>{if(readOnly)throw Error("唯讀工作區不可解除綁定");return routing.unbindGraph(root,scenario);},onBindGraph:(id,scenario)=>{if(readOnly)throw Error('唯讀工作區不可綁定');return routing.bindGraph(root,id,scenario);},onGraphApi:(action,options)=>{if(readOnly&&['save','delete'].includes(action))throw Error('唯讀工作區不可修改圖');return wiringEditor.graphApi(root,action,options);},defaultReq:'Review this implementation',onActivity:()=>workbench('activity'),onListArtifacts:()=>workbench('artifacts'),onOpenArtifact(relative){const file=confined(relative);bridge.postMessage({type:'fixturePreview',url:'/artifact?path='+encodeURIComponent(path.relative(root,file))});}});
...
const server=http.createServer(async(req,res)=>{
  ...
  let messages=[];
  try{const m=JSON.parse(body);if(readOnly&&['saveRoute','preset','graphSave','graphDelete','unbindGraph'].includes(m.type))throw Error('真實工作區唯讀驗證，不修改設定。');
    ...
    else if(m.type==='preset'){await routing.applyPreset(root,m.preset,undefined,{model:m.model});messages=[{type:'catalog',catalog:await routing.getCatalog(root)},{type:'graphList',result:await wiringEditor.graphApi(root,'list')}];}
    else if(m.type==='unbindGraph'){await routing.unbindGraph(root,m.scenario);messages=[{type:'catalog',catalog:await routing.getCatalog(root)}];}
    else if(m.type==='route')messages=[{type:'routePreview',route:await routing.previewRoute(root,m.requirement,undefined,m.scenario)}];
    else if(['graphList','graphGet','graphSave','graphDelete'].includes(m.type)){messages=[await send(m,({graphList:'graphList',graphGet:'graphLoaded',graphSave:'graphSaved',graphDelete:'graphList'})[m.type])];if(m.bind&&messages[0].type==='graphSaved')messages.push({type:'catalog',catalog:await routing.getCatalog(root),selectedScenario:m.graph.scenario});}
