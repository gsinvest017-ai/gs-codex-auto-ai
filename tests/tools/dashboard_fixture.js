...
const readOnly = !!process.argv[3];
const root = process.argv[3] || fs.mkdtempSync(path.join(os.tmpdir(),'codex-ui-qa-'));
const port = Number(process.argv[2] || 8765);
let currentState;
const bridge={html:'',postMessage(message){if(message.type==='state')currentState=message;},onDidReceiveMessage(){return {dispose(){}};}};
const dispose=dashboard.wireDashboard(bridge,{root,defaultReq:'Review this implementation'});
const shim = `<script>
...
window.addEventListener('load',()=>{const label=document.createElement('div');label.textContent='本機 UI 驗證環境：正式檔案讀取與 webview bridge；指定工作區唯讀；啟動任務停用。';label.style='padding:10px;background:#544315;color:white';document.body.prepend(label);const poll=async()=>{const data=await(await fetch('/state')).json();window.dispatchEvent(new MessageEvent('message',{data}));};poll();setInterval(poll,2000);});
</script>`;
...
http.createServer(async(req,res)=>{
  if(req.method==='GET' && req.url==='/state'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(currentState));return;}
  ...
  try {
    ...
    if(readOnly && ['saveRoute','preset'].includes(m.type)) throw new Error('真實工作區唯讀驗證，不修改設定。');
...
}).listen(port,'127.0.0.1',()=>process.stdout.write(JSON.stringify({url:'http://127.0.0.1:'+port,root,readOnly})+'\n'));
