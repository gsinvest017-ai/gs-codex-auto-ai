...
const server=http.createServer(async(req,res)=>{
  ...
  let messages=[];
  try{const m=JSON.parse(body);if(readOnly&&['saveRoute','preset','graphSave','graphDelete'].includes(m.type))throw Error('真實工作區唯讀驗證，不修改設定。');
    ...
    else if(['graphList','graphGet','graphSave','graphDelete'].includes(m.type))messages=[await send(m,({graphList:'graphList',graphGet:'graphLoaded',graphSave:'graphSaved',graphDelete:'graphList'})[m.type])];
